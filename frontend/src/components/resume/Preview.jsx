...
const Preview = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const personalResponse = await axios.get('http://localhost:5000/api/personal');
        setPersonalInfo(personalResponse.data);

        const skillsResponse = await axios.get('http://localhost:5000/api/skills');
        setSkills(skillsResponse.data);

        const experienceResponse = await axios.get('http://localhost:5000/api/experience');
        setExperiences(experienceResponse.data);

        const educationResponse = await axios.get('http://localhost:5000/api/education');
